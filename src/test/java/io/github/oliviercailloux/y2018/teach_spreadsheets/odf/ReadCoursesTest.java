...
public class ReadCoursesTest {
	...
	public void testReadCourses() throws Exception {
		URL is = ReadCoursesTest.class.getResource("Saisie_voeux_dauphine.ods");
		URI uri = is.toURI();
		File file = new File(uri);
		String yearOfStudy = "L3_Informatique";

		ReadCourses rd = new ReadCourses(file);
		Table currentTable = rd.getReader().getDocument().getTableByName(yearOfStudy);
		List<Course> courses;
	...
	/**
	 * Test to check that the method retrieves the right courses from a specified
	 * sheet
	 */
	@Test
	public void testReadCoursesFromSpecifiedSheet() throws Exception {

		URL is = ReadCoursesTest.class.getResource("Saisie_voeux_dauphine_TestForReadCourses_SingleSheet.ods");
		URI uri = is.toURI();
		File file = new File(uri);
		String yearOfStudy = "L3_Informatique";
	...
	public void testReadCoursesFromEverySheet() throws Exception {
		...
		URL is = ReadCoursesTest.class.getResource("Saisie_voeux_dauphine_TestForReadCourses_TwoSheets.ods");
		URI uri = is.toURI();
		File file = new File(uri);
