...
public class WriteCourses {
	...
	private int yearBegin = 0, yearEnd = 0, studentsNumber = 0,
			firstSemesterNumber = 1;
	private ArrayList<Course> firstSemesterCourses = null;
	private ArrayList<Course> secondSemesterCourses = null;

	WriteCourses(File excelFile, String completeYearOfStudyName, int yearBegin,
			int yearEnd, int studentsNumber, int firstSemesterNumber,
			ArrayList<Course> firstSemester, ArrayList<Course> secondSemester)
			throws Exception {
		...
		if (yearBegin < 0 || yearEnd < 0 || yearBegin > yearEnd)
			throw new IllegalArgumentException("Années incorrectes");
		this.yearBegin = yearBegin;
		this.yearEnd = yearEnd;
		if (studentsNumber < 0)
			throw new IllegalArgumentException(
					"Le nombre d'étudiants ne peut pas être négatif");
		this.studentsNumber = studentsNumber;
		if (firstSemesterNumber <= 0)
			throw new IllegalArgumentException(
					"Le numéro du premier semestre est incorrect");
	 ...
	 */
	private void WriteSemesterCourses(ArrayList<Course> coursesList, int col,
	...
	public void WriteCoursesOfYear() throws Exception {
		...
		// Adding new sheet
		Table sheet = this.workbook
				.appendSheet(firstSemesterCourses.get(0).getYearOfStud());

		// Creating template
		// Header
		sheet.getCellByPosition("B1").setFont(headerBoldFont);
		sheet.getCellByPosition("B1")
				.setStringValue(this.completeYearOfStudyName);
		...
		String[] arrayHeader = {"Matière", "code Apogée",
				...
				"Choix TP/CMTP", "Nombre de TD/TP souhaités",
	...
	public ArrayList<Course> getFirstSemesterCourses() {
		return firstSemesterCourses;
	}

	public void setFirstSemesterCourses(
			ArrayList<Course> firstSemesterCourses) {
		this.firstSemesterCourses = firstSemesterCourses;
	}

	public ArrayList<Course> getSecondSemesterCourses() {
		return secondSemesterCourses;
	}

	public void setSecondSemesterCourses(
			ArrayList<Course> secondSemesterCourses) {
	...
	public static void main(String[] args) throws Exception {
		...
		WriteCourses yearOfStudyWriter = new WriteCourses(new File(
				"src/main/resources/Saisie_voeux_dauphine_WriteCourses.ods"),
