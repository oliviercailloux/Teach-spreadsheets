...
public class CourseWriter {
	...
	private final static Logger LOGGER = LoggerFactory
			.getLogger(CourseWriter.class);

	/**
	 * We supposed that source is different from destination.
	 */
	public CourseWriter(SpreadsheetDocument spreadsheetDocument,
	...
	public void writeCoursesOfYear(boolean save) throws Exception {
		...
		// Adding new sheeth
		Table sheet = this.spreadsheetDocument
				.appendSheet(courseSheet.getSheetMetadata().getYearOfStud());
		int yearBegin = Course.getYearBegin();
		int yearEnd = yearBegin + 1;
		// Creating template
		// Header
		writeHeader(headerBoldFont, headerFont, border2, headerColor, sheet,
				yearBegin, yearEnd);
		...
		// Arrays cells Format
		// First Array
		setSheetSemesterFormat(1, cellContentFont, border1, border2,
				firstArraySize, sheet);

		// Second Array
		setSheetSemesterFormat(15, cellContentFont, border1, border2,
				secondArraySize, sheet);
		...
		String[] arrayHeader = {"Matière", "code Apogée",
				...
				"Nombre de groupes indicatif", "Choix Cours", "Choix TD/CMTD",
				"Choix TP/CMTP", "Nombre de groupes souhaités",
		...
		writeSemesterCourses(courseSheet.getCourses(1), START_COLUMN_1,
				START_ROW_1, save);
		writeSemesterCourses(courseSheet.getCourses(2), START_COLUMN_2,
				START_ROW_2, save);
	}
	 ...
	 * @throws Exception
	 */
	private void writeSemesterCourses(List<Course> coursesList, int col,
			int line, boolean save) throws Exception {
		...
		for (Course course : coursesList) {
			...
			if (course.getCMTD_Hour() == 0.0 && course.getTD_Hour() == 0.0) {
				...
				if (course.getCMTD_Hour() != 0.0) {
					sheet.getCellByPosition(col + 5, lineCursor)
							.setStringValue(course.getCMTD_Hour() + "h CMTD");
				} else {
					sheet.getCellByPosition(col + 5, lineCursor)
			...
			if (course.getCMTP_Hour() == 0.0 && course.getTP_Hour() == 0.0) {
				...
				if (course.getCMTP_Hour() != 0.0) {
					sheet.getCellByPosition(col + 6, lineCursor)
							.setStringValue(course.getCMTP_Hour() + "h CMTP");
				} else {
					sheet.getCellByPosition(col + 6, lineCursor)
	...
	private void writeHeader(Font headerBoldFont, Font headerFont,
			Border border2, Color headerColor, Table sheet, int yearBegin,
			int yearEnd) {
		sheet.getCellByPosition("B1").setFont(headerBoldFont);
		sheet.getCellByPosition("B1").setStringValue(
				courseSheet.getSheetMetadata().getCompleteYearOfStudyName());
		...
		sheet.getCellByPosition("I1").setStringValue(
				": " + courseSheet.getSheetMetadata().getStudentNumber()
						+ " étudiants");
		sheet.getCellRangeByPosition("I1", "M1").merge();
		sheet.getCellByPosition("I1").setHorizontalAlignment(
				StyleTypeDefinitions.HorizontalAlignmentType.CENTER);
		sheet.getCellByPosition("I1").setTextWrapped(true);

		sheet.getCellByPosition("B2").setFont(headerBoldFont);
		sheet.getCellByPosition("B2").setStringValue("ENSEIGNEMENTS SEMESTRE "
				+ courseSheet.getSheetMetadata().getFirstSemesterNumber());
		...
		sheet.getCellByPosition("P2").setStringValue("ENSEIGNEMENTS SEMESTRE "
				+ (courseSheet.getSheetMetadata().getFirstSemesterNumber()
						+ 1));
	...
	public void setSheetSemesterFormat(int firstColumnPosition,
			Font cellContentFont, Border border1, Border border2,
			int firstArraySize, Table sheet) {
		for (int i = 2; i <= firstArraySize + 2; i++) {
			for (int j = firstColumnPosition; j <= firstColumnPosition
					+ 12; j++) {
				...
				if (j == firstColumnPosition) {
					...
					sheet.getCellByPosition(j, i).setHorizontalAlignment(
							StyleTypeDefinitions.HorizontalAlignmentType.CENTER);
				if (j == firstColumnPosition + 7
						|| j == firstColumnPosition + 12)
					sheet.getCellByPosition(j, i)
	...
	public void fillHeader(int firstColumn, Color headerColor, Table sheet,
			String[] arrayHeader) {
		for (int j = firstColumn; j <= firstColumn + 12; j++) {
			sheet.getCellByPosition(j, 2)
					.setStringValue(arrayHeader[j - firstColumn]);
